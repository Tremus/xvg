...
#define XVG_MAX_FONT_SLOTS 8
    ...
    // Currently 8 is a required maximum, due to how the shader is written
    // In theory we could support 16 (a typical hardware limit), but I've arbitrarily chosen 8
    XVGAtlas atlases[8];
...
void xvg_gradient_apply_image(XVGGradient* grad, sg_view texture, sg_sampler sampler, int x, int y, int w, int h)
{
    ...
    XVG_ASSERT(w > 0);
    XVG_ASSERT(h > 0);
    grad->xy      = _xvg_pack_xy_coord(x, y);
    grad->wh      = w | (h << 16);
...
{
    XVGBufferRange range;
    range.begin_idx = range.end_idx = xcl->frame.num_line_segments;
    ...
    if (backingScaleFactor == 2)
    {
        ...
        xvg_line_segment_t* dst      = xcl->line_segments + range.begin_idx;
        int                 work_len = N - 2;
        int                 i, j;
        ...
        XVG_ASSERT(range.begin_idx + i == range.end_idx - 2);
        XVG_ASSERT(j == len - 1);
        dst[i].y     = data[j];
        dst[i + 1].y = data[j];
    }
    else
    {
        ...
        memcpy(xcl->line_segments + range.begin_idx, data, N * sizeof(xcl->line_segments[0]));
    }

    xcl->frame.num_line_segments += N;
...
XVGBufferRange _xvg_draw_line_plot_with_gradient_impl(
    ...
    XVGShapeType    shape_type)
{
    XVGBufferRange range = _xvg_add_line(xcl, data, width);
    _xvg_draw_line_plot_ex(xcl, x, y, width, height, crop_br, stroke_width, grad, shape_type, range);
    return range;
}
...
XVGBufferRange xvg_draw_line_plot_with_gradient(
    ...
    XVGGradient     g)
{
    return _xvg_draw_line_plot_with_gradient_impl(xcl, x, y, w, h, data, br, sw, g, XVG_SHAPE_LINE_PLOT);
}

XVGBufferRange
xvg_draw_line_plot(XVGCommandList* xcl, int x, int y, int w, int h, const float* data, float br, float sw, uint32_t col)
{
    XVGGradient grad = {.colour1 = col};
    return _xvg_draw_line_plot_with_gradient_impl(xcl, x, y, w, h, data, br, sw, grad, XVG_SHAPE_LINE_PLOT);
}
...
XVGAtlasRect _xvg_get_atlas_rect(XVG* xcl, uint32_t glyph_index, unsigned font_size, unsigned font_weight)
{
    ...
    for (int j = 0; j < num_rects; j++)
    {
        ...
        if (header_a.data == header_b.data)
        {
            ...
            XVG_ASSERT(rect->y + rect->h < XVG_ATLAS_WIDTH);
            return *rect;
        }
    }
...
void xvg_command_list_end_frame(XVGCommandList* xcl, int window_width, int window_height)
{
    ...
    while (cmd_idx > 0 && cmd_idx < XVG_ARRLEN(xcl->commands) && inf_protection++ < XVG_ARRLEN(xcl->commands))
    {
        ...
        case XVG_CMD_DRAW:
        {
            ...
            if (num_shapes)
            {
                sg_apply_pipeline(xvg->pip_shapes);
            ...
            if (num_text)
            {
                sg_apply_pipeline(xvg->pip_text);
                // clang-format off
                sg_apply_bindings(&(sg_bindings){
                    .views[VIEW_vs_xvg_text_buffer] = xcl->text_sbv,
                    .views[VIEW_fs_xvg_text_atlas1] = xvg->atlases[0].img_view.id ? xvg->atlases[0].img_view : xvg->fallback_img_view,
                    .views[VIEW_fs_xvg_text_atlas2] = xvg->atlases[1].img_view.id ? xvg->atlases[1].img_view : xvg->fallback_img_view,
                    .views[VIEW_fs_xvg_text_atlas3] = xvg->atlases[2].img_view.id ? xvg->atlases[2].img_view : xvg->fallback_img_view,
                    .views[VIEW_fs_xvg_text_atlas4] = xvg->atlases[3].img_view.id ? xvg->atlases[3].img_view : xvg->fallback_img_view,
                    .views[VIEW_fs_xvg_text_atlas5] = xvg->atlases[4].img_view.id ? xvg->atlases[4].img_view : xvg->fallback_img_view,
                    .views[VIEW_fs_xvg_text_atlas6] = xvg->atlases[5].img_view.id ? xvg->atlases[5].img_view : xvg->fallback_img_view,
                    .views[VIEW_fs_xvg_text_atlas7] = xvg->atlases[6].img_view.id ? xvg->atlases[6].img_view : xvg->fallback_img_view,
                    .views[VIEW_fs_xvg_text_atlas8] = xvg->atlases[7].img_view.id ? xvg->atlases[7].img_view : xvg->fallback_img_view,
